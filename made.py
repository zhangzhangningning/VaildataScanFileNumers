    ...
    def __init__( 
        ...
        for i in range(len(self.embeddings)):
            if self.embeddings[i] is not None:
                new_embedding = nn.Embedding(self.embeddings[i].num_embeddings + 1, self.embed_size, padding_idx=0)
                new_embedding.weight.data[1:] = self.embeddings[i].weight.data
    ...
    def forward(self, data):
        ...
        if self.input_encoding == 'embed':
             for i, coli_dom_size in enumerate(self.input_bins):
                # use -1 as not used column
                # Inference pass only 
                if coli_dom_size >= self.embed_size or not self.input_no_emb_if_leq:
                    col_i_embs = self.embeddings[i](data[:, i])
                else:
                    col_i_embs = torch.zeros(data.shape[0], self.embed_size).to(data.device)
                col_i_embs = col_i_embs.unsqueeze(1)
                col_embs.append(col_i_embs)
        else:
            raise ValueError('Unknown input encoding: %s' % self.input_encoding)
        col_embs = torch.cat(col_embs, dim=1).sum(dim=1)
        return col_embs
